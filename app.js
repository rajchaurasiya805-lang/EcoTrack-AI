...
const app = (() => {
  ...
  function calculateCarbonFootprint() {
    // Read input values
    const carDistance = parseFloat(DOM.carDistance.value) || 0;
    const carType = DOM.carType.value;
    const bikeDistance = parseFloat(DOM.bikeDistance.value) || 0;
    const busDistance = parseFloat(DOM.busDistance.value) || 0;
    const trainDistance = parseFloat(DOM.trainDistance.value) || 0;
    const electricityUsage = parseFloat(DOM.electricityUsage.value) || 0;
    const gasBill = parseFloat(DOM.gasBill.value) || 0;
    const householdSize = Math.max(1, parseInt(DOM.householdSize.value) || 1);

    // Sync input object in state
    state.inputs = {
      carDistance,
      carType,
      bikeDistance,
      busDistance,
      trainDistance,
      activeDistance: parseFloat(DOM.activeDistance.value) || 0,
      electricityUsage,
      gasBill,
      householdSize,
      diet: state.inputs.diet // set by diet cards event listeners
    };

    // 1. TRANSPORTATION EMISSIONS (kg CO2e / year)
    ...
    // 2. UTILITY EMISSIONS (kg CO2e / year per person)
    ...
    // 3. DIETARY EMISSIONS (kg CO2e / year)
    const dietTotal = EMISSION_FACTORS.diet[state.inputs.diet] || 1700;
    ...
    // Save to history log (limit to 6 logs max)
    const dateStr = formatDate(new Date());

    // Check if we should update today's log or push new one
    ...
    // Save state & sync UI views
    saveState();
    updateUIElements();

    // Show a success alert message and navigate to Dashboard
    showNotification("Footprint calculated successfully!");
